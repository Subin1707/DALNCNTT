document.addEventListener("DOMContentLoaded", () => {
    ...
    const normalizeVictim = (raw) => {
        if (raw == null) return null;
        const s = String(raw).trim();
        return s ? s : null;
    };

    const handleAnalyze = async (victimRaw) => {
        const victimAccount = normalizeVictim(victimRaw);
        if (!victimAccount) return alert("Vui lòng nhập Victim account");

        try {
            if (resultDiv) resultDiv.innerHTML = "";

            const btn = document.getElementById("analyzeVictimAccountBtn");
            if (btn) btn.disabled = true;

            const res = await fetch("/admin/analyze", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ victimAccount })
            });
                ...
                resultDiv.innerHTML = `
                    ...
                    ">
                        <div style="display:flex;justify-content:space-between;align-items:center">
                            <b>Kết quả phân tích Victim account</b>
            ...
            console.error(err);
            alert(err?.message || "Không thể phân tích Victim account");
        } finally {
            const btn = document.getElementById("analyzeVictimAccountBtn");
    ...
    document.getElementById("analyzeVictimAccountBtn")?.addEventListener("click", () => {
        const val = document.getElementById("analyzeVictimAccount")?.value || "";
        handleAnalyze(val);
    });
});
