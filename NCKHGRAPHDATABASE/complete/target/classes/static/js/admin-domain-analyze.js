document.addEventListener("DOMContentLoaded", () => {
    ...
    const handleAnalyze = async (domainRaw) => {
        const domain = normalizeDomain(domainRaw);
        if (!domain) return alert("Vui lòng nhập Domain");

        try {
            if (resultDiv) resultDiv.innerHTML = "";

            const btn = document.getElementById("analyzeDomainBtn");
            ...
            console.error(err);
            alert(err?.message || "Không thể phân tích Domain");
        } finally {
            const btn = document.getElementById("analyzeDomainBtn");
    ...
    document.getElementById("analyzeDomainBtn")?.addEventListener("click", () => {
        const val = document.getElementById("analyzeDomain")?.value || "";
        handleAnalyze(val);
    });
});
