document.addEventListener("DOMContentLoaded", () => {
    ...
    const handleAnalyze = async (fileNameRaw) => {
        const fileNode = normalizeFileName(fileNameRaw);
        if (!fileNode) return alert("Vui lòng nhập File name");

        try {
            if (resultDiv) resultDiv.innerHTML = "";

            const btn = document.getElementById("analyzeFileNodeBtn");
                ...
                resultDiv.innerHTML = `
                    ...
                    ">
                        <div style="display:flex;justify-content:space-between;align-items:center">
                            <b>Kết quả phân tích File name</b>
            ...
            console.error(err);
            alert(err?.message || "Không thể phân tích File name");
        } finally {
            const btn = document.getElementById("analyzeFileNodeBtn");
    ...
    document.getElementById("analyzeFileNodeBtn")?.addEventListener("click", () => {
        const val = document.getElementById("analyzeFileNode")?.value || "";
        handleAnalyze(val);
    });
});
